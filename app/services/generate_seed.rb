class GenerateSeed
  ...
  PREFIXES = [" SIMPLIE ", " KONTINO ", " BAMTA ", " RATNER ", " EZBO ", " FORMI ", " PEMBA ", " LIDELL ", " STANDOT ", " SHENBON ", " DUGLAS ", " AMBO ", " SNUG ", " SHOLID ", " FILMON ", " HOMATO ", " BENKU ", " STUDIA ", " ZIANY ", " NISKO ", " PIDOR ", " DOMINIK "].freeze
  ...
  TITLES = ["너무 좋아요", "헬로 월드", "이게 간지다", "너무 친절해요", "배송이 느려요", "또 이용할게요", "사장님이 미쳤어요", "상태가 완전 최고에요", "언제 또 입고되나요?", "사이즈 교환요", "정보좀요", "쿨거래 감사요", "항상 애용하고 있습니다", "많이 파세요", "별점 5점 드립니다",
            "칭찬칭찬합니다"].freeze
  CATEGORIES = %w[수납장 침대 테이블 의자 소파 조명 리빙 유아동].freeze
    ...
    def generate_seed
      ...
      CATEGORIES.each_with_index do |category_title, index|
        ...
        create(
          ...
          position: index,
          image: File.open("#{Rails.root}/public/icons/#{index}.jpg")
        )
      end
    end
  end
  ...
  Item.instance_eval do
    def generate_seed
      ...
      50.times do
        ...
        object_hash = {
          name: "마켓큐#{PREFIXES.sample}시리즈",
          list_price: item_price_sample[0],
          sale_price: item_price_sample[1],
          category: Category.all.sample,
          description: Faker::Lorem.sentence(word_count: 30),
          image: File.open(Dir["#{Rails.root}/public/new_seed/*"].sample),
          zipcode: "111111",
          address1: "서울시 #{ADDRESSES.sample}",
          address2: "#{rand(10_000)}호",
        ...
        3.times { object.images.create(image: File.open(Dir["#{Rails.root}/public/new_seed/*"].sample)) }
      end
    end
  end
end
